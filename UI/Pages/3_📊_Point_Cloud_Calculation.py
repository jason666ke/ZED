...
if right_img and left_img is not None:
    ...
    disp = compute_utils.compute_disparity_CRE(left_img, right_img, model)
    ...
    # Display point cloud
    st.info(
        "Due to the inability to visualize 3D point cloud images in Streamlit, we utilize Open3D for visualization.")
    o3d.visualization.draw_geometries([point_cloud])

    # Save point cloud
    if st.button("Save Point Cloud"):
        cur_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = "E:\grade_4\graduate\ZED\\test_data\\"
        file_name = "point_cloud_" + cur_time + ".pcd"
        o3d.io.write_point_cloud(path + file_name, point_cloud)
    ...
    #     'colorB': colors[:, 2]
    # })
    # pcd_df.to_csv("E:\grade_4\graduate\ZED\kochi_pcd.csv")
    # # pcd Layer
    ...
    #     views=[view]
    # )
    # deck.to_html("point_cloud_layer.html")
    # st.pydeck_chart(deck)
    # components.html(deck.to_html())
